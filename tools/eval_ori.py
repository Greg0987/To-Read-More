...
from captioning.data.dataloader_ori import *
from captioning.data.dataloaderraw import *
import captioning.utils.eval_utils_ori as eval_utils
import argparse
...
parser.add_argument('--tags_h5', type=str, default='data/tags_to_calculate.hdf5',
                help='path to tags h5 file')
parser.add_argument('--new_features', type=int, default=0)
opts.add_eval_options(parser)
opts.add_diversity_opts(parser)
opt = parser.parse_args()
print('input_label_h5:',opt.input_label_h5)
...
model.to(opt.device)
model.eval()
crit = losses.LanguageModelCriterion(opt)
...
loss, split_predictions, split_n_predictions, lang_stats = eval_utils.eval_split(
    model, crit, loader, vars(opt)) # 此处的split参数默认为test
