...
def eval_split_n(model, loader, n_predictions, input_data, eval_kwargs={}):
    ...
    if sample_n_method == 'bs':
        ...
        with torch.no_grad():
            model(obj_f, opt=tmp_eval_kwargs, mode='sample')
    ...
    # sample / gumbel / topk sampling / nucleus sampling
        ...
        with torch.no_grad():
            _seq, _sampleLogprobs = model(obj_f, opt=tmp_eval_kwargs, mode='sample')
        # 解码获取句子
...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    while True:
        ...
        obj_f, labels, masks, tags = \
            [data['obj_f'], data['labels'], data['masks'],
             data['tags_calc']]
        obj_f = obj_f.cuda()
        tags = tags.cuda()
        labels = labels.cuda()
        ...
        with torch.no_grad():
            ...
            # 随机采样，生成结果
            seq, seq_logprobs = model(obj_f, opt=tmp_eval_kwargs, mode='sample')
        ...
        if sample_n > 1:    # 多次采样，将多个句子添加到n_predictions中
            eval_split_n(model, loader, n_predictions, [obj_f, data], eval_kwargs)
