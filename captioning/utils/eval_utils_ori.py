...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    tags_vocab = json.load(open('data/cocotalk.json', 'r'))['ix_to_word']
    ...
    while True:
        ...
        tmp = [data['fc_feats'], data['att_feats'], data['tags_calc'],data['labels'], data['masks'], data['att_masks']]
        tmp = [_.to(device) if _ is not None else _ for _ in tmp]
        fc_feats, att_feats, tags, labels, masks, att_masks = tmp
        ...
        if labels is not None and verbose_loss:
            ...
            with torch.no_grad():
                out = model(fc_feats, att_feats, labels[..., :-1], att_masks)
        ...
        with torch.no_grad():
            ...
            for i in range(nn):
                _sentence = [vocab[str(word.item())] for word in seq[i] if word != 0]   # 0是填充符号
                # _sentence_set = set(_.item() for _ in seq[i] if _ != 0]
                tags_set = set(tags_vocab[str(tag.item())] for tag in tags[i] if tag != 0)
                # print('=' * 30)
                # print(_sentence)
                # print(tags_set)
