...
def save_checkpoint(opt, model, infos, optimizer, histories=None, append=''):
    ...
    # opt.checkpoint_path = os.path.join('./outputs', opt.checkpoint_path)
    if not os.path.isdir(os.path.join('./outputs', opt.checkpoint_path)):
        os.makedirs(os.path.join('./outputs', opt.checkpoint_path))
    checkpoint_path = os.path.join('./outputs', opt.checkpoint_path, 'model%s.pth' %(append))
    torch.save(model.state_dict(), checkpoint_path)
    print("model saved to {}".format(checkpoint_path))
    optimizer_path = os.path.join('./outputs', opt.checkpoint_path, 'optimizer%s.pth' %(append))
    torch.save(optimizer.state_dict(), optimizer_path)
    with open(os.path.join('./outputs', opt.checkpoint_path, 'infos_'+opt.id+'%s.pkl' %(append)), 'wb') as f:
        pickle_dump(infos, f)
    if histories:
        with open(os.path.join('./outputs', opt.checkpoint_path, 'histories_'+opt.id+'%s.pkl' %(append)), 'wb') as f:
