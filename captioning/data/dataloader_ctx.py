...
class Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.k = getattr(opt, 'topk', 12)   # The nums of tags to use
    ...
    def collate_func(self, batch, split):   # 对一个batch的样本进行处理和组合
        ...
        if self.new_features:
            ...
            for sample in batch:
                ...
                tag_1_f, tag_5_f, tag_5_p, tag_9_f, tag_9_p = tmp_tag

                tag_1_embed_batch.append(tag_1_f)
                tag_1_pos_batch = torch.zeros((len(tag_1_embed_batch), len(tag_1_embed_batch[0])), dtype=torch.long)
                tag_5_embed_batch.append(tag_5_f)
                tag_5_pos_batch.append(tag_5_p)
                tag_9_embed_batch.append(tag_9_f)
                tag_9_pos_batch.append(tag_9_p)
            ...
            data['grid_f'] = torch.from_numpy(np.stack(grid_batch))

            data['tag_f'] = {'whole': {"embed": torch.stack(tag_1_embed_batch), "pos": tag_1_pos_batch},
                             'five': {"embed": torch.stack(tag_5_embed_batch), "pos": torch.stack(tag_5_pos_batch)},
                             'nine': {"embed": torch.stack(tag_9_embed_batch), "pos": torch.stack(tag_9_pos_batch)}}
    ...
    def __getitem__(self, index):
        ...
        if self.new_features:
            ...
            tag_5_f = tag_feat["five"]["features"][:,:self.k,:]
            tag_5_p = np.tile(np.arange(5)[:, None], (1, self.k))

            tag_9_f = tag_feat["nine"]["features"][:,:self.k,:]
            tag_9_p = np.tile(np.arange(9)[:, None], (1, self.k))

            tag_1_f = torch.FloatTensor(tag_1_f)
            tag_5_f = torch.FloatTensor(tag_5_f.reshape((5*self.k, -1)))
            tag_5_p = torch.LongTensor(tag_5_p.reshape((5*self.k,)))
            tag_9_f = torch.FloatTensor(tag_9_f.reshape((9*self.k, -1)))
            tag_9_p = torch.LongTensor(tag_9_p.reshape((9*self.k,)))

            tag = (tag_1_f, tag_5_f, tag_5_p, tag_9_f, tag_9_p)
