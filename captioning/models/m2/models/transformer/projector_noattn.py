...
class SampleAttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        return out, q_
        # 经过attention的v，以及降维过后的q
        # (b_s, nq, d_model), (b_s, q_after, d_k)

# 包一下attention作layernorm
class SampleAttention(nn.Module):
    def __init__(self, d_model, d_k, d_v, q_before, q_after, d_reduction=None, dropout=0.3):
        super(SampleAttention, self).__init__()
        self.attention = SampleAttentionLayer(d_model, d_k, d_v, q_before, q_after, d_reduction, dropout)
        self.layer_norm = nn.LayerNorm(d_model)

    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        # 经过attention的q，和原始的q(可能经过降维)
        out, q = self.attention(queries, keys, values, attention_mask, attention_weights)
        out = self.layer_norm(out + q)
        return out

# 映射类，将O和T进行处理拼接
class Projector_noattn(nn.Module):
    def __init__(self, f_obj, f_grid, f_tag, f_out, topk, d_reduction=None, drop_rate=0.3):
        ...
        # self.obj_self_attn = SampleAttention(d_model=512, d_k=64, d_v=64,
        #                                  q_before=50, q_after=50,
        #                                  d_reduction=None, dropout=drop_rate)
        # self.obj_cross_attn = SampleAttention(d_model=512, d_k=64, d_v=64,
        #                                     q_before=50, q_after=50,
        #                                     d_reduction=None, dropout=drop_rate)
        # self.ffn_obj = nn.Sequential(   # 全连接层2，包含线性连接层、dropout层
    ...
    def forward(self, obj, grid, tag):
        ...
        # 模态0
        x = 0
        obj_embed = torch.cat((x * torch.ones((obj_embed.shape[0],obj_embed.shape[1], 1)).to(obj_embed), obj_embed), dim=-1)  # (b_s, 1+f_out)
        embed_obj.append(obj_embed)
        embed_obj_ = torch.cat(embed_obj, dim=1)  # (b_s, 50, 512)

        # embed_obj__ = self.obj_self_attn(embed_obj_, embed_obj_, embed_obj_)  # (b_s, 50, 512)
        ...
        for k in self.keys:
            ...
            embed_k = torch.cat((x * torch.ones((embed_k.shape[0],embed_k.shape[1], 1)).to(embed_k), embed_k), dim=-1)  # (b_s, 1+f_out) # 加上模态1
            embed_grid.append(embed_k)
        embed_grid_ = torch.cat(embed_grid, dim=1)  # (b_s, 1+4+9+16+25, 512)

       #  embed_obj_attn = self.obj_cross_attn(embed_obj__, embed_grid_, embed_grid_)  # (b_s, 50, 512)
        # embed_obj_ = self.layer_norm(self.ffn_obj(embed_obj_attn) + embed_obj_attn)  # (b_s, 50, 512)
        ...
        for k in self.keys:
            ...
            # embed_k = pos_drop(embed_k + pos_embed)
            embed_k = torch.cat((x * torch.ones((embed_k.shape[0],embed_k.shape[1], 1)).to(embed_k), embed_k), dim=-1)  # (b_s, 1+f_out)
        ...
        # # 此处的tag_attn是经过交叉注意力后的结果，已经包含了grids的信息
