...
class Projector_attn(nn.Module):
    def __init__(self, f_obj, f_grid, f_tag, f_out, topk, drop_rate=0.3):
        ...
        for k in self.keys:
            ...
            pos_embed = nn.Parameter(torch.zeros(1, num_embeddings * topk, f_out))  # 位置编码
            pos_drop = nn.Dropout(p=drop_rate)
            nn.init.trunc_normal_(pos_embed, std=0.02)
        ...
        # for co-attention
        self.ln1 = nn.LayerNorm(f_out)
        self.ln2 = nn.LayerNorm(f_out)
        self.ln3 = nn.LayerNorm(f_out)
        self.self_attn = MultiHeadAttention(d_model=512, d_k=64, d_v=64, h=8, dropout=drop_rate,
                                        identity_map_reordering=False,
                                        attention_module=None,
                                        attention_module_kwargs=None)

        self.cross_attn = MultiHeadAttention(d_model=512, d_k=64, d_v=64, h=8, dropout=drop_rate,
                                        identity_map_reordering=False,
                                        attention_module=None,
                                        attention_module_kwargs=None)
        self.ffn = nn.Sequential(
            nn.Linear(512, 2048), nn.ReLU(), nn.Dropout(p=drop_rate), nn.Linear(2048, 512))
    ...
    def forward(self, obj, grid, tag):
        ...
        embed_tag_ = self.ln1(self.self_attn(embed_tag_, embed_tag_, embed_tag_) + embed_tag_)
        embed_tag_ = self.ln2(self.cross_attn(embed_tag_, embed_grid_, embed_grid_) + embed_tag_)
        embed_tag_ = self.ln3(self.ffn(embed_tag_) + embed_tag_)
