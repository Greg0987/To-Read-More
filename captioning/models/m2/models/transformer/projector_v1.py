...
class Projector(nn.Module):
    def __init__(self, f_obj, f_grid, f_tag, f_out, drop_rate=0.3, k=9):
        ...
        self.keys = ("whole", "nine", "sixteen", "tweentyfive")
        self.k = k
        ...
        for k in self.keys:
            ...
            input_index = torch.arange(num_embeddings)
            setattr(self, f"tag_index_{k}", input_index)
    ...
    def forward(self, obj, grid, tag):
        ...
        # tags
        for k in self.keys:
            embed_k = tag[k]    # -> (b_s, 9*k, d)
            mlp_2 = getattr(self, f"tag_mlp_{k}")
            mlp_pos = getattr(self, f"tag_pos_{k}")
            tag_index = getattr(self, f"tag_index_{k}")
            tag_index = tag_index.to(embed_k.device)
            tag_index = tag_index.repeat(1, self.k)  #                     重复k次，得到(b_s, 9*k)

            embed_k = mlp_2(embed_k) + mlp_pos(tag_index)   # tag向量 + 位置编码  # 【待确定一下能否实现广播机制】
            embed.append(embed_k)
