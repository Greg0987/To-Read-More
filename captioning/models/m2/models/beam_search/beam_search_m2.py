...
class BeamSearchM2(object):
    ...
    def apply(self, obj, out_size=1, return_probs=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                obj, outputs = self.iter(t, obj, outputs, return_probs, **kwargs)
                # print('第{}步，生成的词为为{}'.format(t, outputs))
        ...
        outputs = outputs.contiguous()[:, :out_size]    # 选前outsize个进行输出
    ...
    def iter(self, t: int, obj, outputs, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        # step在第0步时，输入初始特征，输出dec(bos, attn)
        # step在t>0步时，输出dec(selected_words, attn)，即下一个词概率
        word_logprob = self.model.step(
            t, self.selected_words, obj, None, mode='feedback', **kwargs
        )
        word_logprob = word_logprob.view(self.b_s, cur_beam_size, -1)   # 词概率

        # 当t==0时，形状为（b_s, 1, 1)，即bos
        candidate_logprob = self.seq_logprob + word_logprob # 先前的概率+当前计算下一个词的概率相加
        ...
        if t > 0:
            ...
            # 创建一个新张量，与cand相同，但是除了第一列以外，将所有元素设置为-999
            old_seq_logprob = self.seq_logprob.expand_as(candidate_logprob).contiguous()
            old_seq_logprob[:, :, 1:] = -999

            # 未结束的位置保持原始概率值（即candiadate_logprob）,结束位置的概率加上-999
            candidate_logprob = self.seq_mask * candidate_logprob + old_seq_logprob * (1 - self.seq_mask)


        selected_idx, selected_logprob = self.select(t, candidate_logprob, **kwargs)    # 从候选词中，选出几个
