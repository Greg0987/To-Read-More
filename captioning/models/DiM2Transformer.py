...
from .m2.models.transformer import Transformer, MemoryAugmentedEncoder, MeshedDecoder, ScaledDotProductAttentionMemory, Projector_attn, Projector_noattn

from .TransformerModel import TransformerModel

class DiM2TransformerModel(TransformerModel):

    def __init__(self, opt):
        super(DiM2TransformerModel, self).__init__(opt)
    ...
    def make_model(self, src_vocab, tgt_vocab, N_enc=3, N_dec=3,
        ...
        if self.opt.d_reduction == 'noattn':
            projector = Projector_noattn(
                f_obj=2054, f_grid=768, f_tag=512, f_out=d_model, topk=self.opt.topk, d_reduction=self.opt.d_reduction,
                drop_rate=dropout
            )
        else:
            projector = Projector_attn(
                f_obj=2054, f_grid=768, f_tag=512, f_out=d_model, topk=self.opt.topk, d_reduction=self.opt.d_reduction,
                drop_rate=dropout
            )
