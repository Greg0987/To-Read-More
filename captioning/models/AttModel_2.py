...
class AttModel(CaptionModel):
    ...
    def _diverse_sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        beam_size = opt.get('beam_size', 1)
        temperature = opt.get('temperature', 1.0)
        group_size = opt.get('group_size', 1)   # 5
        ...
        # 生成序列，（group_size, batch_size, seq_length）
        trigrams_table = [[] for _ in range(group_size)] # will be a list of batch_size dictionaries

        # 序列表，（group_size, batch_size, seq_length）
        seq_table = [fc_feats.new_full((batch_size, self.seq_length), self.pad_idx, dtype=torch.long) for _ in range(group_size)]
        # 序列概率表，（group_size, batch_size, seq_length）
        seqLogprobs_table = [fc_feats.new_zeros(batch_size, self.seq_length) for _ in range(group_size)]

        # 状态表，（group_size, batch_size, hidden_size）
        state_table = [self.init_hidden(batch_size) for _ in range(group_size)] # (divm, batch_size, -1)

        for tt in range(self.seq_length + group_size):  # 遍历每个时间步
            for divm in range(group_size):  # 遍历每个group
                t = tt - divm   # t是当前时间步
                seq = seq_table[divm]                   # 当前group的序列
                seqLogprobs = seqLogprobs_table[divm]   # 当前group的序列概率
                trigrams = trigrams_table[divm]         # 当前group的trigrams
                if t >= 0 and t <= self.seq_length-1:
                    ...
                    # Add diversity
                    if divm > 0:
                        unaug_logprobs = logprobs.clone()   # 原始概率
        ...
        _seq = torch.stack(seq_table, 1).reshape(batch_size * group_size, -1)
        _seqLogprobs = torch.stack(seqLogprobs_table, 1).reshape(batch_size * group_size, -1)

        self.done_beams = [[{} for _ in range(group_size)] for _ in range(batch_size)]
        for k in range(batch_size):
            for _n in range(group_size):
                self.done_beams[k][_n]['seq'] = _seq[k * group_size + _n]
                self.done_beams[k][_n]['logps'] = _seqLogprobs[k * group_size + _n]
        print(len(self.done_beams[0]))
